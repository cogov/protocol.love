...
remove = function (a, b) {return checkForError('remove', _core_remove(a, b))}
var _core_makeHash = makeHash
makeHash = function (a, b) {return checkForError('makeHash', _core_makeHash(a, b))}
var _core_debug = debug
debug = function (a) {return checkForError('debug', _core_debug(JSON.stringify(a)))}
var _core_call = call
call = function (a, b, c) {return __holochain_api_check_for_json(checkForError('call', _core_call(a, b, c)))}
var _core_commit = commit
commit = function (a, b) {return checkForError('commit', _core_commit(a, b))}
var _core_get = get
get = function (a, b) {return __holochain_api_check_for_json(checkForError('get', b === undefined ? _core_get(a) : _core_get(a, b)))}
var _core_getLinks = getLinks
getLinks = function (a, b, c) {return checkForError('getLinks', _core_getLinks(a, b, c))}
var _core_send = send
send = function (a, b, c) {return __holochain_api_check_for_json(checkForError('send', c === undefined ? _core_send(a, b) : _core_send(a, b, c)))}
function __holochain_api_check_for_json(rtn) {
	try {rtn = JSON.parse(rtn)} catch (err) {}
	return rtn
}
function checkForError(func, rtn) {
	if (typeof rtn === 'object' && rtn.name == 'HolochainError') {
		var errsrc = new getErrorSource(4)
		var message = 'HOLOCHAIN ERROR! "' + rtn.message.toString() + '" on ' + func + (errsrc.line === undefined ? '' : ' in ' + errsrc.functionName + ' at line ' + errsrc.line + ', column ' + errsrc.column + '\n\n' + errsrc.stack)
		throw{ name: 'HolochainError', function: func, message: message, holochainMessage: rtn.message, source: errsrc, toString: function () {return this.message} }
	}
	return rtn
}
...
function createCollective(parms) {
	...
	try {
		c = get(makeHash('cogov_collective', parms.name))
	} catch (err) {
		if (err.holochainMessage != 'hash not found') throw err
	}
	if (c !== undefined)
	...
	commit('cogov_collective_link', {
		Links: [
			{
				Base: App.DNA.Hash,
				Link: c,
				Tag: 'cogov_collective'
			}
		]
	})
...
function getMembers(parms) {
	...
	var members = []
	try { var members = getLinks(parms.collectiveId, 'cogov_member', { Load: true }) } catch (err) { if (err.holochainMessage !== 'No links for cogov_member') throw err }
	debug(members)
	var rtn = []
...
function validateAddMember(parms) {
	...
	try { members = getLinks(parms.collectiveId, 'cogov_member', {}) } catch (err) { if (err.holochainMessage != 'No links for cogov_member') throw err }
	for (var x = 0; x < members.length; x++)
		if (members[x].Hash == m)
...
function confirmCollective(collectiveId) {
	...
	try { var entryType = get(collectiveId, { GetMask: HC.GetMask.EntryType }) } catch (err) {
...
function commitItemDetails(base, name, value) {
	commit('cogov_item_details_link', {
		Links: [
			{
				Base: base,
				Link: commit(
...
function getActionId(base) {
	try { var actionIds = getLinks(base, 'cogov_item_details_actionId', { Load: true }) } catch (err) { debug(err) }
...
function validateLink(linkingEntryType, baseHash, linksArray, tag, pkg, sources) {
	...
	if (linkingEntryType == 'cogov_member_link') {
		...
		try { validateAction(action) } catch (err) {
			debug('FAILED!')
			return false
		}
		return true
	...
	if (linkingEntryType == 'cogov_action_link') {
		try { validateAction(get(linksArray[0].Link)) } catch (err) {
			debug('FAILED!')
			return false
		}
		return true
