#
# Test and build protocol.love DNA
#
...
# `make test` directly, or build a target directly eg. `nix-build -A protocol.love`

SHELL		= bash
DNANAME		= protocol.love
DNA		= $(DNANAME).dna
HAPP		= $(DNANAME).happ
WASM		= target/wasm32-unknown-unknown/release/protocol.love.wasm
...
$(WASM): FORCE
	...
	@RUST_BACKTRACE=1 CARGO_TARGET_DIR=target cargo build --release --target wasm32-unknown-unknown
...
test-dna:
	@echo "Starting Scenario tests in $$(pwd)..."; \
	    cd test && ./node_modules/.bin/pnpm test

test-dna-debug:
	@echo "Starting Scenario tests in $$(pwd)..."; \
	    cd test && ./node_modules/.bin/pnpm run test-debug

test-e2e:	test-node test-dna

test-node:
	@echo "Setting up Scenario/Stress test Javascript..."; \
	    cd test && ./node_modules/.bin/pnpm install && cd ..
